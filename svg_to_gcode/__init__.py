...
SETTING 	= {
    ...
    # Toolparameters
    "pass_depth",           # positive integer	sets tool cutting depth in machine units (Z-axis: the depth your laser cuts in a pass; note
                            #                   that this depends on laser power and material!)
...
def check_setting(setting: dict[str,Any] =None) -> bool:
    ...
    for key in setting.keys():
        ...
        if key in {"minimum_laser_power","maximum_laser_power","x_axis_maximum_rate","y_axis_maximum_rate", "movement_speed"
                   "x_axis_maximum_travel","y_axis_maximum_travel"} and (not isinstance(setting[key],int) or setting[key] < 0):
        ...
        if key == "rapid_move" and setting[key] not in {True,False}:
            raise ValueError(f"Unknown '{key}' value '{setting[key]}'. Please specify one of the following: {{True,False}}")
        if key == "fan" and setting[key] not in {True,False}:
            raise ValueError(f"Unknown '{key}' value '{setting[key]}'. Please specify one of the following: {{True,False}}")
